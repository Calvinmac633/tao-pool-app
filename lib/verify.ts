/**
 * Consistency checks over a wallet's analytics. Used by `npm run verify`
 * locally and by GET /api/verify on a hosted instance.
 */
import { unexplainedTolerance } from "./ledger-analytics";
import type { PositionAnalytics, WalletAnalytics } from "./types";

export type Check = { name: string; ok: boolean; detail: string };
export type VerifyReport = { wallet: string; asOf: string | null; lines: string[]; checks: Check[]; failures: number };

const f = (n: number) => n.toFixed(2);

export function runChecks(d: WalletAnalytics): VerifyReport {
  const lines: string[] = [];
  const checks: Check[] = [];
  ...
  lines.push(`wallet ${d.wallet}, as of ${d.asOf}, ${d.open.length} open, ${d.closed.length} closed shown`, "");

  const checkPosition = (a: PositionAnalytics) => {
    lines.push(`${a.closedAt ? "closed" : "open  "} ${a.positionId.slice(0, 8)} ${a.poolName}  value ${f(a.lastUsdValue)}  price ${f(a.lastPrice)}`);
  ...
  return { wallet: d.wallet, asOf: d.asOf, lines, checks, failures };
}
